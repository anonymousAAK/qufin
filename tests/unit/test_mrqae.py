...
class TestEuropeanProblemRegression:
    ...
    ``build_european_estimation_problem`` must use a unitary StatePreparation
    (``prepare_state``) so that ``A^dag`` exists; otherwise every QAE algorithm
    crashes when constructing the Grover operator on a European problem.
    """
